...
[x] 71 Phase 71 - Workbench Route Role And Return-Path Convergence
  ...
  [x] 71.1 Section - Parent-Context And Return-Path Cleanup
    ...
    [x] 71.1.1 Task - Normalize repo-detail parent context
      ...
      [x] 71.1.1.1 Subtask - Ensure dashboard-originated repo-detail links
      ...
      [x] 71.1.1.2 Subtask - Keep explicit Workbench-originated flows returning
        to Workbench only when the operator actually came from the dense
        specialist route.
      [x] 71.1.1.3 Subtask - Align breadcrumb and back-label wording with the
        actual parent route so repo detail no longer hints that Workbench is the
        canonical signed-in home.

    [x] 71.1.2 Task - Normalize specialist follow-up routes
      ...
      [x] 71.1.2.1 Subtask - Preserve actual parent context through
        conversation, memory, semantic, workflow, and governed-run handoff links
        instead of hard-coding one historical parent route.
      [x] 71.1.2.2 Subtask - Keep direct URL entry resilient by choosing a
        sensible product-owned fallback that matches the newer dashboard-first
        product model.
      [x] 71.1.2.3 Subtask - Avoid client-only navigation memory so route state
        stays testable and reproducible.

  [x] 71.2 Section - Final Workbench Route Role
    ...
    [x] 71.2.1 Task - Choose the stable route behavior
      ...
      [x] 71.2.1.1 Subtask - If Workbench remains a route, keep it clearly
        bounded as a denser specialist mode with shared dashboard content and no
        separate top-level taxonomy.
      [x] 71.2.1.2 Subtask - If Workbench becomes an alias or redirect, ensure
