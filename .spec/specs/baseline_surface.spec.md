...
kind: feature
status: active
summary: jido_code exposes a state-aware `/welcome` landing and auth entry route, keeps `/setup` as the signed-in continuation surface for incomplete onboarding, uses `/dashboard` as the durable ready-state authenticated landing through left-sidebar concern navigation with a repository-first overview rendered as split monitoring panels plus bounded in-place accordions, and keeps operator auth and Git integration management on `/settings/auth` while authenticated product, API, and dev surfaces remain declared in the router.
decisions:
