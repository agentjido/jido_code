...
<!-- current_truth.reconciled_with_branch: durable-memory writer maintenance stays governed by the capture-plane contract described here, including the newer conversation-turn provenance envelopes, explicit conversation-memory adoption seam, and retrieval-oriented provenance metadata that later bounded recall surfaces depend on without bypassing the canonical write boundary. -->
...
kind: feature
status: active
summary: Jido.Code inserts memory-graph individuals through a bounded memory capture plane that accepts typed capture envelopes instead of raw triples, records workflow provenance at AgentWorkspace and workflow-boundary transitions into `workflow_provenance`, routes any long-term productive conversation-history capture into that same provenance lane before memory adoption, records durable classified memories into `memory` only through explicit product or governed adoption paths, now includes typed workflow-provenance envelope normalization plus canonical writer boundaries for both workflow provenance and durable memory, adds explicit conversation-turn provenance capture plus conversation-memory adoption seams for later bounded recall, now emits typed governed relations from those writers while leaving artifact-style compatibility data internal to the cutover, adds typed durable-memory update envelopes plus a canonical update writer for validation, invalidation, and supersession, keeps explicit record/query/validate/invalidate/refresh and repository-scoped recovery workspace entrypoints so callers stop assuming direct store writes, updates freshness and invalidation metadata when revision or test evidence changes, requires explicit repository, work-item, workspace, actor, and revision context for any durable insertion, now supports product-owned memory inspection, retrieval, and adoption surfaces that still emit typed capture requests instead of bypassing the canonical write seam, extends that same seam to current operator memory actions and governed workflow follow-up rather than allowing direct graph mutation from richer product surfaces, and now defines a canonical typed governed-reference contract that callers emit as `governed_references` while any artifact-style compatibility data stays internal to the cutover.
decisions:
...
- id: architecture.memory_capture_plane.scenario_governed_reference_contract_replaces_generic_artifact_naming
  covers:
    - architecture.memory_capture_plane.typed_governed_reference_contract_is_canonical
    - architecture.memory_capture_plane.product_and_runtime_callers_emit_capture_envelopes_not_raw_triples
  given:
    - Memory, provenance, or product services need to point at governed product records.
  when:
    - The semantic stack normalizes those links before later write or query behavior.
  then:
    - The links use a typed governed-reference contract with kind, id, iri, and label semantics.
    - Generic artifact-path naming is no longer treated as the durable semantic contract.
```
...
- kind: source_file
  target: lib/jido_code/memory_graph/governed_reference.ex
  covers:
    - architecture.memory_capture_plane.product_and_runtime_callers_emit_capture_envelopes_not_raw_triples
    - architecture.memory_capture_plane.typed_governed_reference_contract_is_canonical
...
- kind: source_file
  target: test/jido_code/memory_graph_governed_reference_test.exs
  covers:
    - architecture.memory_capture_plane.product_and_runtime_callers_emit_capture_envelopes_not_raw_triples
    - architecture.memory_capture_plane.typed_governed_reference_contract_is_canonical
