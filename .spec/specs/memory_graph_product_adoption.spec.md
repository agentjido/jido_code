...
<!-- current_truth.reconciled_with_branch: surface feedback, memory components, governed live surfaces, repo-detail cohosting now through a distinct route-owned `Memory` family beside the `Conversations` family, repo-detail memory readiness now reading the managed repository's repo-scoped workspace binding instead of setup-wide defaults, blocked memory states now using the same repo-scoped workspace-binding vocabulary and repair path as adjacent conversation, semantic, and workflow panels on repo detail, with the adjacent blocked conversation notice now exposing that repair jump inline, setup import now accepting explicit repo-scoped local workspace paths without one shared parent root while setup-host-shell runtime-default copy now frames `workspace_root` as import-seed metadata, phase-62 integration coverage now proving those repo-scoped bindings stay canonical while unbound repos remain explicitly blocked, adjacent multi-repository setup follow-up that clears completed selections, repo-owned onboarding-reset coverage that now clears imported managed-repository inventory plus repo-scoped kernel snapshots, and the shared LiveView route harness now using a settings-owned `/settings/auth` destination for provider-login and Git integration management plus a sibling `/settings/github` add-repository flow that imports canonical managed repositories while dashboard remains the ready-state authenticated entry surface, now projects bounded memory readiness through dashboard `Work > Overview` plus `/workbench` via the shared managed-repository inventory model instead of leaving dashboard overview empty, now shows bounded conversation-origin recall on repo detail and governed run detail with cross-links back to canonical conversation routes instead of exposing transcript internals on memory surfaces, those recall cards now using stable route-owned ids and browser-covered canonical links, keeps those signed-in memory surfaces under one shared product-wide operator navigation layer, now lands the proportional breadcrumb-and-pane shell on `/workbench` and governed run detail while settings reuses the same shell primitives through route-owned child-subject navigation, and now proves that adjacent shared shell plus bounded recall routing through route and browser coverage instead of leaving memory-adjacent routes visually special-cased. -->
