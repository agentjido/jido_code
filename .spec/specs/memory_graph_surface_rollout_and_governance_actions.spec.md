...
<!-- current_truth.reconciled_with_branch: governed memory helpers plus work-item, evidence, decision, repo-detail cohosting now through route-owned `Memory` and `Semantic` families beside the canonical `Conversations` family, repo-detail graph-backed readiness now sharing the same repo-scoped workspace-binding contract as the managed repository route, with blocked graph-backed states now using the same repo-scoped repair vocabulary and route-local repair path as adjacent conversation and workflow surfaces and the blocked conversation notice now surfacing that repair jump inline, setup import now accepting explicit repo-scoped local workspace paths without one shared parent root while neighboring setup-route host-shell copy now frames runtime defaults as seed metadata only, adjacent shared setup/auth regression coverage now keeps GitHub-backed repository validation on full accessible listings instead of GitHub's default 30-result first page, phase-62 integration coverage now proving those repo-scoped bindings stay canonical while unbound repos remain explicitly blocked, adjacent multi-repository setup follow-up that clears completed selections, repo-owned onboarding-reset coverage that now clears imported managed-repository inventory plus repo-scoped kernel snapshots, and the shared LiveView route harness now using a settings-owned `/settings/auth` destination for provider-login and Git integration management plus a sibling `/settings/github` add-repository flow that imports canonical managed repositories while dashboard remains the ready-state authenticated entry surface, now keeps bounded memory summaries in both the dedicated dashboard memory concern and the shared dashboard `Work > Overview` inventory projection, now lets repo-detail and governed-run memory surfaces project bounded conversation-origin recall plus cross-links back to canonical conversation routes without rehosting transcripts there, those recall cards now using stable route-owned ids that browser coverage follows back into the canonical conversation panel, keeps those surfaces under one shared signed-in operator navigation layer, lands the proportional shared shell on `/workbench` and governed run detail instead of inventing fake graph navigation, and now proves that adjacent-route shell plus bounded recall routing through shared route and browser coverage. -->
...
kind: feature
status: active
summary: Jido.Code now extends memory-graph product adoption beyond repository detail and run detail into dashboard summaries plus canonical work-item, evidence, and decision surfaces, uses product-owned services and routed LiveView boundaries to host bounded memory and workflow-provenance context on those routes, makes validate, invalidate, supersede, and promote actions available from those same canonical surfaces rather than through special-case graph views, keeps memory-aware summaries and follow-up suggestions bounded and action-oriented, standardizes cross-graph navigation among memory, provenance, source code, governed records, and canonical conversation routes across the remaining operator surfaces, ensures memory-aware workflow or governed follow-up continues to consume product-shaped projections instead of raw graph responses or direct pod interactions, keeps repo-detail graph-backed readiness aligned to the managed repository's repo-scoped workspace binding, now keeps dashboard memory context behind a dedicated authenticated sidebar-selected dashboard memory concern plus the shared dashboard `Work > Overview` inventory projection instead of leaving overview empty, lands the proportional shared shell on `/workbench` and governed run detail, reuses proportional operator-shell helpers when adjacent signed-in routes adopt shared breadcrumb and pane framing, including repositories imported with explicit repo-scoped local paths outside one shared root, later repaired through a repo-scoped workspace-binding update boundary, and introduced through setup copy that now describes install-wide defaults as seed metadata only, while dashboard summary feeds, governed surface sections, follow-up preview widgets, typed governed references, and bounded conversation-origin recall stay tied to canonical product routes and the shared capture-envelope seam, including repo detail where route-owned `Memory` and `Semantic` families now keep those cross-links on one managed-repository route and expose them through stable route-owned ids that browser coverage can exercise.
decisions:
