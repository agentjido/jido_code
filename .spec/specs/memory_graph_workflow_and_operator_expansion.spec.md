...
<!-- current_truth.reconciled_with_branch: governed-route memory actions, cross-graph navigation, follow-up provenance, repo-detail cohosting now through route-owned `Memory` and `Semantic` families beside the canonical `Conversations` family, repo-detail graph-backed operator surfaces now sharing the managed repository's repo-scoped workspace-binding contract for readiness and recovery, with blocked graph-backed operator states now using the same repo-scoped repair vocabulary and route-local repair path as adjacent conversation and workflow panels and the blocked conversation notice now surfacing that repair jump inline, repo-detail memory plus governed-run follow-up now project bounded conversation-origin recall cards with canonical conversation links instead of surfacing transcript bodies on memory or governed panes, setup import now accepting explicit repo-scoped local workspace paths without one shared parent root while shared setup-route host-shell copy now frames runtime defaults as seed-only metadata, phase-62 integration coverage now proving those repo-scoped bindings stay canonical while unbound repos remain explicitly blocked, adjacent multi-repository setup follow-up that clears completed selections, repo-owned onboarding-reset coverage that now clears imported managed-repository inventory plus repo-scoped kernel snapshots, and the shared LiveView route harness now using a settings-owned `/settings/auth` destination for provider-login and Git integration management plus a sibling `/settings/github` add-repository flow that imports canonical managed repositories while dashboard remains the ready-state authenticated entry surface, now exposes bounded memory-driven follow-up from dashboard `Work > Overview` plus `/workbench` through the shared managed-repository inventory model, keeps those signed-in operator routes under one shared global navigation layer, now lands the proportional shared shell on `/workbench` and governed run detail, and now proves adjacent-route shell consistency plus bounded recall routing through shared route and browser coverage instead of leaving workflow follow-up visually special-cased. -->
...
kind: feature
status: active
summary: Jido.Code now hosts bounded repository memory and workflow-provenance context across canonical governed run, work-item, evidence, and decision surfaces, routes validate, invalidate, supersede, and promote follow-up actions through product-owned memory boundaries over the capture plane rather than raw graph mutation, standardizes bounded cross-graph navigation among memory, workflow provenance, source-code entities, governed records, and canonical conversation routes through the companion control-plane ontology plus typed governed references, lets planner, coder, reviewer, and explainer flows request durable memory through explicit retrieval policies keyed to freshness, memory class, provenance scope, and follow-up intent, and keeps memory-derived behavior explainable by preserving freshness, supersession, provenance, governed follow-up previews, bounded conversation-origin recall, recovery affordances, governed adoption metadata, and repo-scoped workspace-binding readiness whenever operators or workflows act on durable memory, including repositories imported with explicit repo-scoped local paths, repositories later repaired through repo-scoped workspace-binding updates that affect only the selected managed repository from the same route, dashboard summary routing that now lives under the authenticated route-owned dashboard concern tabs while dashboard `Work > Overview` plus `/workbench` reuse the shared managed-repository inventory projection for bounded memory-driven follow-up instead of leaving overview empty, `/workbench` and governed run detail now adopting the proportional shared shell, and repo detail where route-owned `Memory` and `Semantic` families now preserve bounded cross-links without collapsing back into one mixed graph surface.
decisions:
