...
<!-- current_truth.reconciled_with_branch: branch-wide implementation and verification surfaces continue to roll up into this package-level subject, including setup-owned secret-encryption readiness helpers, PAT preflight UI behavior, bounded multi-repository GitHub import follow-up on the signed-in setup surface, GitHub-backed repository validation that now widens the initial API page to 100 repositories and continues pagination beyond the first page so linked repository selection does not truncate at GitHub's default 30-result page, the signed-in setup selector and its fallback now grouping linked repositories by account origin while keeping the account name visible on each card, adding route-local account-owner filtering plus matching repository search in both richer and fallback setup paths, the rule that completed GitHub imports persist import history without staying selected as active follow-up work, the repo-owned onboarding reset path that now clears imported managed-repository inventory plus persisted repo-kernel snapshots while preserving non-onboarding deployment credentials, the later `/settings/github` add-repository flow now routing through the same canonical managed-repository import boundary instead of a settings-only row path, the repo-scoped workspace-binding cutover that now allows explicit per-repository local workspace paths without one shared parent root plus a product-owned repo-scoped rebinding service and repo-detail repair surface that update only the selected managed repository, the signed-in setup copy that now frames `workspace_root` as seed metadata for new imports, the converged repo-detail runtime wording that now keeps conversations, semantic, memory, and workflow panels on the same repo-scoped readiness and repair story with inline conversation-repair affordances on blocked repo-detail runtime notices, the landed post-onboarding subject-tree shell on dashboard and managed-repository detail, the new bounded conversation-origin recall cards on repo-detail memory and governed-run follow-up panes that reopen canonical repo conversations instead of embedding transcript browsers, the shared browser harness reset and scenario seeding needed to cover that route behavior deterministically, the shared signed-in global operator navigation layer across dashboard, workbench, settings, repository inventory, managed-repository detail, governed run detail, and adjacent operator routes, the clarified route role that now keeps `/workbench` as a dense specialist mode rather than a competing ready-state landing surface, the shared proportional operator-shell helper layer now landed on `/workbench`, `/repos`, `/workflows`, `/agents`, and governed run detail, the shared shell primitive reuse now landed on settings child-subject navigation with pane footers rendered only when the selected settings concern owns an action, the adjacent operator-route LiveView and browser coverage that now proves those signed-in shells behave consistently, the welcome/bootstrap handoff ADR plus phased routing/settings plan, the ready-state local-auth and provider-auth cutover that now defaults authenticated product entry to dashboard while incomplete onboarding still routes to setup, the signed-in `/welcome` handoff card that now prioritizes dashboard and settings before a compact auth-settings cue, the new settings-owned `/settings/auth` operator configuration destination plus README and CONTRIBUTING route-orientation guidance, and the routed repo-detail conversation shell plus browser-harness verification work that now keeps clarification, readiness, degraded continuity, route-local workspace repair, and bounded recall cross-linking under version-controlled package surfaces. -->
...
- kind: source_file
  target: .spec/planning/phase-73-adjacent-signed-in-route-shell-adoption.md
  covers:
    - package.jido_code.spec_led_workspace
