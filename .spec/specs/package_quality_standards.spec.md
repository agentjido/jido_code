...
<!-- current_truth.reconciled_with_branch: contributor-facing quality docs remain version controlled and now include route-owned conversation host and readiness guidance, the provenance-first conversation-recall rule for memory and governed surfaces, explicit routed entry ownership for `/welcome`, `/setup`, `/dashboard`, and `/settings/auth`, clearer onboarding-reset command guidance that includes imported managed-repository cleanup during local bootstrap rewinds, while repo-owned browser verification continues to cover both setup and conversation Playwright surfaces. -->
...
kind: policy
status: active
summary: jido_code keeps the contributor-facing quality and core Mix command surfaces from the Jido package standards current, including repo-local compatibility package wiring, a repo-local Doctor config for generated modules without source files, committed dependency and lockfile hygiene that keeps `mix q` actionable after consolidated refreshes even as the source-code graph ontology, memory-graph ontology, TripleStore semantic stack add heavier native dependencies, a repo-owned `mix server` start alias that prepares the current browser stack before boot, a direct repo-owned Dotenvy dependency that supports local dev runtime bootstrap without changing the production secret posture, Vite-backed frontend asset commands that still flow through repo-owned Mix aliases, a deeper `mix frontend.verify` browser verification command, repo-owned `mix source_graph.verify`, `mix memory.verify`, `mix semantic.verify`, and onboarding-reset commands, with the reset command surface now clearing imported managed repositories plus repo-scoped kernel snapshots during local bootstrap rewinds, contributor docs that describe both the approved LiveView-plus-LiveVue browser boundary and the routed entry split across `/welcome`, `/setup`, `/dashboard`, and `/settings/auth`, the repository-scoped source graph plus memory graph workflow as bounded enhancements, the provenance-first conversation-recall rule for memory and governed surfaces, the companion ontology pair, the typed governed-reference cutover, the local dotenv bootstrap contract, and a documented fast-vs-deep quality split while the repo carries existing Dialyzer and Doctor debt, plus the current repo-local Spec Led gate surface in docs and CI.
decisions:
...
- id: package.jido_code.package_quality_alignment_doc_present
  statement: "The repository shall keep its package-quality alignment, approved browser-composition conventions, product-oriented fallback messaging expectations, local dev runtime bootstrap conventions, and any explicit product-repo exceptions documented in version-controlled repo files or specs."
  priority: must
  stability: stable
...
- kind: command
  target: "rg -n 'mix server|mix frontend.verify|mix source_graph.verify|mix memory.verify|mix semantic.verify|LiveView-plus-LiveVue|<\\.vue_surface|source_code graph|memory graph|semantic graph|bounded enhancement' README.md CONTRIBUTING.md AGENTS.md"
  covers:
    - package.jido_code.package_quality_alignment_doc_present
    - package.jido_code.package_quality_mix_surface_aligned
