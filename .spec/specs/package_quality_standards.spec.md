...
kind: policy
status: active
summary: jido_code keeps the contributor-facing quality and core Mix command surfaces from the Jido package standards current, including repo-local compatibility package wiring and the current repo-local Spec Led gate surface in docs and CI, while documenting explicit product-repo exceptions.
decisions:
