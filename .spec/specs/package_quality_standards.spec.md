...
kind: policy
status: active
summary: jido_code keeps the contributor-facing quality and core Mix command surfaces from the Jido package standards current, including repo-local compatibility package wiring, a repo-local Doctor config for generated modules without source files, committed dependency and lockfile hygiene that keeps `mix q` actionable after consolidated refreshes even as the source-code graph ontology/store stack adds heavier native dependencies, a repo-owned `mix server` start alias that prepares the current browser stack before boot, Vite-backed frontend asset commands that still flow through repo-owned Mix aliases, a deeper `mix frontend.verify` browser verification command, repo-owned `mix source_graph.verify` and `mix semantic.verify` semantic graph verification commands, contributor docs that describe both the approved LiveView-plus-LiveVue browser boundary and the repository-scoped source graph workflow as a bounded enhancement, and a documented fast-vs-deep quality split while the repo carries existing Dialyzer and Doctor debt, plus the current repo-local Spec Led gate surface in docs and CI.
decisions:
...
- kind: command
  target: "rg -n 'mix server|mix frontend.verify|mix source_graph.verify|mix semantic.verify|LiveView-plus-LiveVue|<\\.vue_surface|source_code graph|semantic graph|bounded enhancement' README.md CONTRIBUTING.md AGENTS.md"
  covers:
    - package.jido_code.package_quality_alignment_doc_present
    - package.jido_code.package_quality_mix_surface_aligned
