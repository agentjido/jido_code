...
kind: policy
status: active
summary: jido_code keeps the contributor-facing quality and core Mix command surfaces from the Jido package standards current, including repo-local compatibility package wiring, a repo-local Doctor config for generated modules without source files, committed dependency and lockfile hygiene that keeps `mix q` actionable after consolidated refreshes even as the source-code graph ontology, memory-graph ontology, and TripleStore semantic stack add heavier native dependencies, a repo-owned `mix server` start alias that prepares the current browser stack before boot, Vite-backed frontend asset commands that still flow through repo-owned Mix aliases, a deeper `mix frontend.verify` browser verification command, repo-owned `mix source_graph.verify`, `mix memory.verify`, and `mix semantic.verify` semantic verification commands, contributor docs that describe both the approved LiveView-plus-LiveVue browser boundary and the repository-scoped source graph plus memory graph workflow as bounded enhancements, the companion ontology pair, the typed governed-reference cutover, and a documented fast-vs-deep quality split while the repo carries existing Dialyzer and Doctor debt, plus the current repo-local Spec Led gate surface in docs and CI.
decisions:
