...
<!-- current_truth.reconciled_with_branch: the routed settings shell now hosts a dedicated `/settings/auth` destination for provider-login and Git integration management, the `/settings/github` add-repository flow now uses the canonical managed-repo import boundary while still propagating the current authenticated actor through settings-owned repo-anchor writes, signed-in operator routes now share one product-wide LiveView-owned navigation layer plus shared shell primitives instead of bespoke route-local top-level chrome, settings pane footers now appear only when the selected settings concern owns a route-level action, adjacent shell coverage now proves that route-owned navigation and actions remain inside the same LiveView policy membrane, and those operator surfaces continue to route through product-owned LiveView actions instead of introducing a separate admin route family or bypassing the existing policy membrane. -->
...
- id: architecture.policy_layers.runtime_capacity_limits_fail_closed
  statement: Product-owned runtime entrypoints shall fail closed with typed capacity outcomes when a repository-scoped runtime queue or concurrency limit is reached instead of silently spawning unbounded work.
  priority: should
  stability: evolving
