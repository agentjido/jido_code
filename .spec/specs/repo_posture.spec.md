...
<!-- current_truth.reconciled_with_branch: run-detail posture coverage continues to coexist with bounded memory context on canonical operator surfaces, including bounded conversation-origin recall that reopens canonical repo conversations when operators need origin continuity, dashboard now keeps runtime and posture summaries inside a dedicated sidebar-selected concern, dashboard `Work > Overview` plus `/workbench` now project bounded runtime posture cues through the shared managed-repository inventory model instead of leaving overview empty on the authenticated landing route, and those signed-in posture surfaces now share one product-wide operator navigation layer with governed run detail also using the shared proportional pane framing. -->
...
kind: feature
status: active
summary: Jido.Code observes repo-native `.spec/` and optional Git-native planning state as durable signals, then projects explainable `RepoPosture` and `PostureCheck` records that stay linked to observations, assessments, evidence, bounded dashboard and run-detail memory context, optional bounded conversation-origin recall on governed follow-up surfaces, canonical managed-repository scope, the authenticated dashboard's dedicated sidebar-selected runtime and posture concern, the shared dashboard `Work > Overview` plus `/workbench` inventory projection that now carries bounded runtime cues instead of an empty overview shell, and the shared signed-in operator navigation layer plus governed-run pane framing that keep those posture surfaces reachable without relying on breadcrumb-only movement, with route-level integration and browser coverage keeping dashboard posture projection explainable instead of hiding trust state inside opaque service logic.
decisions:
