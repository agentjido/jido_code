...
kind: feature
status: active
summary: Jido.Code observes repo-native `.spec/` and optional Git-native planning state as durable signals, then projects explainable `RepoPosture` and `PostureCheck` records that stay linked to observations, assessments, evidence, bounded dashboard and run-detail memory context, canonical managed-repository scope, the authenticated dashboard's dedicated sidebar-selected runtime and posture concern, the shared dashboard `Work > Overview` plus `/workbench` inventory projection that now carries bounded runtime cues instead of an empty overview shell, and the shared signed-in operator navigation layer plus governed-run pane framing that keep those posture surfaces reachable without relying on breadcrumb-only movement, with route-level integration and browser coverage keeping dashboard posture projection explainable instead of hiding trust state inside opaque service logic.
decisions:
