...
<!-- current_truth.reconciled_with_branch: run detail continues to keep runtime narratives product-oriented alongside bounded memory context, while dashboard runtime posture now renders behind a dedicated left-sidebar runtime concern and dashboard `Work > Overview` plus `/workbench` now project bounded runtime cues through the shared managed-repository inventory model instead of keeping overview empty on the authenticated route, with those runtime surfaces now also sharing the same signed-in operator navigation layer and proportional shell framing as adjacent dashboard, settings, and deep follow-up routes. -->
