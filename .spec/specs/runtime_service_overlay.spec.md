...
<!-- current_truth.reconciled_with_branch: run detail continues to keep runtime narratives product-oriented alongside bounded memory context and bounded conversation-origin recall that still routes transcript continuity back through canonical repo conversations, while dashboard runtime posture now renders behind a dedicated left-sidebar runtime concern and dashboard `Work > Overview` plus `/workbench` now project bounded runtime cues through the shared managed-repository inventory model instead of keeping overview empty on the authenticated route, with those runtime surfaces now also sharing the same signed-in operator navigation layer and proportional shell framing as adjacent dashboard, settings, and deep follow-up routes. -->
...
kind: policy
status: deprecated
summary: Jido.Code treats jido_os as an authority-backed runtime-services overlay composed of public service facades and optional admitted capabilities, while product-owned gateways preserve stable product contracts, governed run detail may co-host bounded memory context, bounded conversation-origin recall, cross-graph follow-up preview, and runtime posture language without collapsing those concerns into one semantic lane, dashboard runtime posture remains a bounded concern behind its own authenticated left-sidebar runtime concern while dashboard `Work > Overview` plus `/workbench` now project bounded runtime cues through the shared managed-repository inventory model instead of an empty overview, those runtime-facing signed-in routes share one LiveView-owned operator navigation layer plus proportional shell framing, route-level integration/browser coverage proves that current routed dashboard contract stays product-oriented, and Ash-backed product truth remains canonical.
