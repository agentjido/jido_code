...
<!-- current_truth.reconciled_with_branch: `/setup` remains the LiveView-owned signed-in start surface with bounded hybrid follow-up work, runtime-default copy now frames `workspace_root` as seed metadata for new local imports rather than a permanent shared-root rule, ready-state local auth now defaults to dashboard, signed-in `/welcome` behaves as a compact handoff surface instead of a setup surrogate, a settings-owned `/settings/auth` destination now carries durable provider-login and Git integration management without changing setup ownership, setup import can now accept explicit repo-scoped local workspace paths instead of assuming one shared parent root, GitHub repository follow-up now supports route-local account-owner filtering plus matching repository search in both the richer widget and the server-rendered fallback without rewriting persisted onboarding selection state, the repo-owned onboarding reset path now clears imported managed-repository inventory plus repo-scoped kernel snapshots while preserving non-onboarding deployment credentials, post-import runtime surfaces now keep blocked repo-scoped workspace remediation on the managed-repository route instead of redirecting operators back through setup defaults, including inline repo-detail conversation repair jumps for missing local workspace bindings, the shared browser scenario harness now resets optional memory-graph state after setup handoff and seeds bounded conversation-origin recall only for scenarios that explicitly need signed-in repo-detail memory coverage, setup completion now hands operators into a dashboard landing whose `Work > Overview` pane is the primary managed-repository inventory home while `/workbench` remains the denser specialist mode and `/repos`, `/workflows`, and `/agents` now share the proportional signed-in shell under the landed post-onboarding navigation layer, and adjacent operator-route browser coverage now proves that signed-in shell stays legible after setup hands work off to the product. -->
...
- id: setup.onboarding.scenario_reset_mix_task_rewinds_or_restarts_setup
  ...
  given:
    - A local install has bootstrap or signed-in onboarding state, imported managed repositories, repo-scoped runtime snapshots, and an onboarding-managed GitHub PAT fallback.
  when:
    - The operator runs `mix onboarding.reset --keep-owner` or `mix onboarding.reset --full`.
  then:
    - `--keep-owner` preserves the bootstrap owner, clears imported managed-repository inventory plus repo-scoped kernel snapshots, clears the onboarding-managed GitHub PAT fallback, and rewinds onboarding to the signed-in `/setup` surface.
    - `--full` clears local bootstrap users, clears imported managed-repository inventory plus repo-scoped kernel snapshots, clears the onboarding-managed GitHub PAT fallback, and returns the install to first-run bootstrap.
