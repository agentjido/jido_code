...
<!-- current_truth.reconciled_with_branch: source-code graph store maintenance remains governed by the pod and load lifecycle described here, including the stable code anchors that newer conversation-provenance envelopes and bounded recall surfaces reuse when conversation-origin context points back to repository entities. -->
...
kind: feature
status: active
summary: Jido.Code provides a repository-scoped SourceCodeGraphPod that analyzes a managed repository with ElixirOntologies in full mode, stages ontology schema plus extracted project individuals as one semantic snapshot, loads that snapshot into the canonical `source_code` named graph of a local TripleStore quad store, assigns stable repository-scoped IRIs so code entities can anchor sibling memory and workflow-provenance graphs later, now feeds those stable IRIs into the typed memory-capture envelope seam for workflow provenance links, durable-memory anchors, and conversation-origin provenance when code entities are known, preserves bounded repository-scoped readiness, explicit stale-revision state, latest failure metadata, degraded stale-query behavior, and recovery entrypoints through AgentWorkspace, supports explainable sibling memory-graph consistency by keeping those code anchors stable across refresh and recovery while the sibling memory stack adds a companion governed control-plane ontology and typed governed references without introducing a fourth graph, keeps governed-surface memory context, bounded conversation-origin recall, and cross-graph navigation anchored to the same stable source-code IRIs instead of ad hoc graph joins, keeps semantic status product-owned even as repository kernels restore persisted runtime state, relies on explicit AgentWorkspace-managed workspace binding rather than ambient runtime state, exposes both explicit SPARQL query actions and compiled semantic helper actions for pod-local specialists and product-owned semantic service boundaries, feeds bounded product-owned semantic projections instead of direct pod internals, grants selected coding specialists explicit semantic lookup tools only through bounded composition, routes higher-level workflow semantic inputs through product-owned workspace entrypoints rather than pod topology, and now relies on typed governed references at the capture-envelope seam for governed links emitted by semantic workflow and adoption helpers, while keeping the phase and workspace verification suite aligned to the persisted-kernel runtime path that now backs source-graph pod restoration.
decisions:
...
- id: architecture.source_code_graph_pod.stable_code_links_support_governed_memory_navigation
  statement: Stable repository-scoped source-code IRIs shall remain usable by product-owned memory and governed-surface services so run-detail memory context and cross-graph navigation can link back to code without exposing raw graph joins or store-local identifiers as product contracts.
  priority: should
  stability: proposed
...
- kind: source_file
  ...
  covers:
    - architecture.source_code_graph_pod.code_entities_use_stable_iris_for_cross_graph_links
    - architecture.source_code_graph_pod.stable_code_links_support_governed_memory_navigation
