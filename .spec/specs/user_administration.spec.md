...
<!-- current_truth.reconciled_with_branch: bootstrap-admin creation and entry into the signed-in setup start surface remain current truth here, while ready-state local auth now defaults to dashboard instead of reopening `/welcome`, later GitHub PAT plus multi-repository GitHub import follow-up still depend on setup-owned server gating and do not keep completed imports selected as a new bootstrap gate, and durable provider-login or Git integration management now has a settings-owned `/settings/auth` destination instead of relying only on welcome-page operator controls. -->
