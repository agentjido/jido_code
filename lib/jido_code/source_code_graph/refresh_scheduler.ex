defmodule JidoCode.SourceCodeGraph.RefreshScheduler do
  ...
  def enqueue(%{managed_repo_id: managed_repo_id} = event) when is_binary(managed_repo_id) do
    if Application.get_env(:jido_code, :source_code_graph_auto_refresh_enabled, true) do
  ...
  def handle_info({ref, result}, %{refresh_task: %{ref: ref}} = state) do
    ...
    if next_state.pending_after_refresh? do
      ...
      {:noreply, persist_scheduler_state(%{next_state | pending_event: nil})}
    end
  end
  ...
  defp enqueue_event(%{refresh_task: task} = state, event) when not is_nil(task) do
    %{
      state
      | pending_event: merge_events(state.pending_event, event),
  ...
  defp enqueue_event(state, event) do
    state
    |> Map.put(:pending_event, merge_events(state.pending_event, event))
    |> schedule_refresh()
  end
  ...
  defp schedule_refresh(state) do
    %{state | refresh_timer: Process.send_after(self(), :run_refresh, state.refresh_debounce_ms), state: :queued}
  end
  ...
  defp merge_events(nil, event), do: normalize_event(event)

  defp merge_events(existing, event) do
    event = normalize_event(event)

    existing
    |> Map.put(:observed_at, event.observed_at)
    |> Map.put(:changed_paths, merge_lists(existing.changed_paths, event.changed_paths))
  ...
  defp normalize_event(event) do
    ...
    %{
      ...
      changed_paths: changed_paths |> Enum.reject(&is_nil/1) |> Enum.uniq() |> Enum.sort(),
