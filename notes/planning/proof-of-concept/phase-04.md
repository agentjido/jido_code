...
Implement the complete flow from Enter key to agent invocation.

- [ ] 4.1.1.1 In TUI update, handle `:submit` by extracting input_buffer content
- [ ] 4.1.1.2 Clear input_buffer and add user message to messages list
- [ ] 4.1.1.3 Set agent_status to `:processing`
- [ ] 4.1.1.4 Lookup LLMAgent via Registry
- [ ] 4.1.1.5 Classify query for CoT using QueryClassifier
- [ ] 4.1.1.6 Dispatch async task calling appropriate agent function
- [ ] 4.1.1.7 Return updated Model with pending response indicator
- [ ] 4.1.1.8 Write end-to-end test: input → agent → response (success: full round-trip works)
...
- [ ] 4.2.1.3 Implement `/model <provider>:<model>` command
- [ ] 4.2.1.4 Implement `/provider <provider>` command (keeps current model family)
- [ ] 4.2.1.5 Implement `/config` to display current configuration
- [ ] 4.2.1.6 Implement `/help` listing available commands
- [ ] 4.2.1.7 Return command result or error message for TUI display
- [ ] 4.2.1.8 Write command parsing tests (success: all commands parse correctly)

### 4.2.2 Model Switching
- [ ] **Task 4.2.2 Complete**

Execute model configuration changes and update agent.

- [ ] 4.2.2.1 On `/model` command, parse provider and model name
- [ ] 4.2.2.2 Validate provider is supported (anthropic, openai, openrouter)
- [ ] 4.2.2.3 Validate API key exists for provider via Keyring
- [ ] 4.2.2.4 Call `LLMAgent.configure/2` with new settings
- [ ] 4.2.2.5 Broadcast `{:config_changed, config}` on success
- [ ] 4.2.2.6 Display success/error message in TUI
- [ ] 4.2.2.7 Write integration test for model switching (success: subsequent queries use new model)
